...
```{r, eval = TRUE}
install.packages(c("leaflet", "rgeos", "rgdal"), repos='http://cran.us.r-project.org')
#install.packeges("gpclib"")
```
...
## Downloading data for bike paths Barcelona
...
Loading the Shapefile in memory

```{r, eval = TRUE}
# load up area shape file:
library(maptools)
area <- readShapePoly("carril_bici.shp")
```
